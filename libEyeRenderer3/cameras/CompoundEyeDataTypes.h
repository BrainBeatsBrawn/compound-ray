...
#include <vector_types.h> // float3 etc
#include "GenericCameraDataTypes.h"

struct CompoundEyeData
{
    std::size_t ommatidialCount = 0;        // The number of ommatidia in this eye
    CUdeviceptr d_ommatidialArray = 0; // Points to a list of Ommatidium objects in VRAM
    std::uint32_t samplesPerOmmatidium = 1; // The number of samples taken from each ommatidium for this eye
